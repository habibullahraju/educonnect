...
const montserratMediumFontUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/fonts/montserrat/Montserrat-Medium.ttf`;
const montserratMidiumFontBytes = await fetch(montserratMediumFontUrl).then(
  (res) => res.arrayBuffer()
);
...
async function GET(request) {
  try {
    ...
    if (!completionDate) {
      return new NextResponse(`The course is not completed.`, {
        status: 500,
      });
    }
    ...
    const montserrat = await pdfDoc.embedFont(montserratMidiumFontBytes);
